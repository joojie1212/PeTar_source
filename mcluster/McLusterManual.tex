...
Option & Range & Meaning\\ \hline
     ...
     -m & $m>0$ & Stellar mass for equal-mass mode ($f=0$); otherwise IMF mass limits ($\msun$), \\
			& & used multiple times from low to high mass\\
      --blackhole & --- & Set every generated object to a black hole (disabled by default)\\
...
\begin{enumerate}
\item All stars can have the same mass (option \texttt{-f0}). Specify the mass of each star in solar masses by using \texttt{-m} once, for example \texttt{-f0 -m30}. If \texttt{-m} is omitted, the \texttt{single\_mass} value in the main routine is used.
\item The optional \texttt{--blackhole} flag sets every generated object to stellar type $kw=14$, radius $0.01\,R_\odot$, zero spin and zero luminosity. It is disabled by default.
