...
#ifdef BHMERGER
            const std::string fbhmerger_name =
                fname_snp + ".bhmerger";
            hard_manager.ar_manager.interaction.bhmerger_output.open(
                fbhmerger_name,
                input_parameters.append_switcher.value == 1,
                my_rank == 0,
                WRITE_WIDTH,
                WRITE_PRECISION);
            // Rank 0 must finish creating the shared file and its header before
            // another rank can append a merger event.
            PS::Comm::barrier();
#endif
#endif 
...
#ifdef BHMERGER
        interaction.bhmerger_output.close();
#endif
#endif
#ifdef ADJUST_GROUP_PRINT
