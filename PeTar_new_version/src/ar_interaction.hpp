...
#ifdef BHMERGER
                // ======================= BHMERGER-ZJ: apply remnant spin =======================
                // In BSEManager::merge particle 1 is retained and particle 2 is set to zero
                // mass. Only apply out[0].spin for that mapping; out[1] belongs to the
                // disappearing particle and must not modify a physical remnant.
                const double mass1_after_zj = bse_manager.getMass(p1->star);
                const double mass2_after_zj = bse_manager.getMass(p2->star);
                if (mass1_after_zj > 0.0 && mass2_after_zj == 0.0) {
                    double dspin1_zj[3];
                    bse_manager.getSpinChange(dspin1_zj, out[0]);
                    p1->spin.x += dspin1_zj[0];
                    p1->spin.y += dspin1_zj[1];
                    p1->spin.z += dspin1_zj[2];
                }
                // ===================== BHMERGER-ZJ: apply remnant spin end =====================
#endif
            ...
            if (_bin_interrupt.status!=AR::InterruptStatus::merge&&_bin_interrupt.status!=AR::InterruptStatus::destroy) {
                ...
                auto merge = [&](const Float& dr, const Float& t_peri, const Float& sd_factor) {
                    ...
                    if (stellar_evolution_option>0) {
                        ...
                        Float semi = _bin.semi;
                        Float ecc = _bin.ecc;
                        //std::cerr << "BEGIN BSE MANAGER"<<std::endl;//zhujie debug
#ifdef BHMERGER
                        ...
                        // ======================= BHMERGER-ZJ: pass orbital vectors to BSE =======================
                        // Direction convention exported to BSEManager::merge:
                        //   dr = r1 - r2
                        //   dv = v1 - v2
                        // The calculateRemnant() direction mismatch is corrected in bse_interface.h.
                        double dr_zj[3], dv_zj[3], spin1_zj[3], spin2_zj[3];
                        for (int k=0; k<3; k++) {
                            dr_zj[k] = p1->pos[k] - p2->pos[k];
                            dv_zj[k] = p1->vel[k] - p2->vel[k];
                        }
                        spin1_zj[0] = p1->spin.x;
                        spin1_zj[1] = p1->spin.y;
                        spin1_zj[2] = p1->spin.z;
                        spin2_zj[0] = p2->spin.x;
                        spin2_zj[1] = p2->spin.y;
                        spin2_zj[2] = p2->spin.z;
                        bse_manager.merge(p1->star, p2->star, out[0], out[1], semi, ecc,
                                          dr_zj, dv_zj, spin1_zj, spin2_zj,
                                          gravitational_constant);
                        // ===================== BHMERGER-ZJ: pass orbital vectors to BSE end =====================
#else
                        bse_manager.merge(p1->star, p2->star, out[0], out[1], semi, ecc);
#endif
                        //std::cerr << "BEGIN postprocess"<<std::endl;//zhujie debug
                        //std::cout<<"postProcess2"<<std::endl;
                        postProcess(out, pos_cm, vel_cm, semi, ecc, 0);
#ifdef BHMERGER
                        // Exactly one particle survives a successful merger. At this
                        // point its remnant mass, CM position, recoil velocity and spin
                        // have all been applied by postProcess().
...
#endif
                        //std::cerr << "BEGIN settime merger"<<std::endl;//zhujie debug

                        if (stellar_evolution_write_flag&&(p1->mass==0.0||p2->mass==0.0)) {
                            //std::cout << "enter if"<<std::endl;
#pragma omp critical(merge_logging)
                            {
                                // Diagnostic: record why Dynamic_merge logging may be skipped
                                std::cerr<<"DEBUG Dynamic_merge invoked: write_flag="<<stellar_evolution_write_flag
                                         <<" m1="<<p1->mass<<" m2="<<p2->mass<<std::endl;
                                if (fout_bse.is_open()) {
                                    fout_bse<<"DEBUG Dynamic_merge invoked: write_flag="<<stellar_evolution_write_flag
                                            <<" m1="<<p1->mass<<" m2="<<p2->mass<<std::endl;
                                }
