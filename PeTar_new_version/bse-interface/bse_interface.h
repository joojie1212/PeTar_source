...
#ifdef BHMERGER
#include "../bhmerger/inspiralZJ.hpp"
#endif
...
struct StarParameterOut{
    ...
    double dm;   ///> mass loss
#ifdef BHMERGER
    // ======================== BHMERGER-ZJ: spin-change output ========================
    double spin[3]; ///> Cartesian spin change applied to the particle: spin_new - spin_old
    // ====================== BHMERGER-ZJ: spin-change output end ======================
#endif
...
#ifdef BHMERGER
             // ======================== BHMERGER-ZJ: spin output columns ========================
             <<std::setw(_width)<<"dspin.x"
             <<std::setw(_width)<<"dspin.y"
             <<std::setw(_width)<<"dspin.z"
             // ====================== BHMERGER-ZJ: spin output columns end ======================
#endif
             ;
    }
    ...
    void printColumn(std::ostream & _fout, const int _width=20) const{
        _fout<<std::setw(_width)<<kw0
             ...
             <<std::setw(_width)<<dm
#ifdef BHMERGER
             // ========================= BHMERGER-ZJ: print spin change =========================
             <<std::setw(_width)<<spin[0]
             <<std::setw(_width)<<spin[1]
             <<std::setw(_width)<<spin[2]
             // ======================= BHMERGER-ZJ: print spin change end =======================
#endif
             ;
    }
...
public:
...
#ifdef BHMERGER
    // ========================== BHMERGER-ZJ: spin-change getter ==========================
    //! Copy the Cartesian spin increment recorded by a stellar-evolution event.
    void getSpinChange(double* _dspin, const StarParameterOut& _out) {
        for (int k=0; k<3; k++) _dspin[k] = _out.spin[k];
    }
    // ======================== BHMERGER-ZJ: spin-change getter end ========================
#endif
    ...
    */
    void merge(StarParameter& _star1, StarParameter& _star2, StarParameterOut& _out1, StarParameterOut& _out2, double& _semi, double& _ecc
#ifdef BHMERGER
               ,
               const double* _dr=nullptr, const double* _dv=nullptr,
               const double* _spin1=nullptr, const double* _spin2=nullptr,
               const double _gravitational_constant=1.0
#endif
        ) {
        ...
        const double total_mass_before_merge = _star1.mt + _star2.mt;
        // ======================== BHMERGER-ZJ: black-hole guard ========================
        // BSE stellar type 14 denotes a black hole. Save this before merge_() changes
        // either component so the remnant fitting model is used only for BH-BH mergers.
        const bool is_black_hole_merger = (_star1.kw == 14 && _star2.kw == 14);
        // ====================== BHMERGER-ZJ: black-hole guard end ======================
#endif

        double semi_rsun = _semi*rscale;

#ifdef BHMERGER_INTERFACE_LINKED
        // ======================= BHMERGER-ZJ: remnant calculation hook =======================
        // Required direction convention:
        //   ar_interaction passes dr = r1 - r2 and dv = v1 - v2.
        //   calculateRemnant currently forms L as v_rel x r_rel, opposite to r_rel x v_rel.
        //   Therefore dv is negated here so it receives (-dv) x dr = dr x dv.
        // The returned value is intentionally only captured here; the later physical updates
        // to _star*, _out*, vkick, spin, or mass are left for the next step.
        BHMerger::RemnantParameters remnant_zj{};
        const bool remnant_zj_input_valid =
            (is_black_hole_merger
             && _dr != nullptr && _dv != nullptr
             && _spin1 != nullptr && _spin2 != nullptr);
        bool remnant_zj_success = false;
        if (remnant_zj_input_valid) {
            remnant_zj_success =
                BHMerger::calculateRemnant(_star1.mt, _star2.mt,
                                           _dr[0], _dr[1], _dr[2],
                                           -_dv[0], -_dv[1], -_dv[2],
                                           _spin1[0], _spin1[1], _spin1[2],
                                           _spin2[0], _spin2[1], _spin2[2],
                                           _gravitational_constant, getSpeedOfLight());
            remnant_zj = BHMerger::getRemnant();
            remnant_zj_success =
                remnant_zj_success
                && std::isfinite(remnant_zj.mfin)
                && remnant_zj.mfin > 0.0;
        }
        // ===================== BHMERGER-ZJ: remnant calculation hook end =====================
#endif
        ...
        _star1.m0 = m0[0];
#ifdef BHMERGER_INTERFACE_LINKED
        // ======================== BHMERGER-ZJ: remnant mass ========================
        // Only a BH-BH merger with a valid fit result overrides the BSE mass.
        // precession.remnantmass() returns M_final/(m1+m2), not a mass in Msun.
        _star1.mt = remnant_zj_success
                  ? remnant_zj.mfin * total_mass_before_merge
                  : mt[0];
        // ====================== BHMERGER-ZJ: remnant mass end ======================
#else
        _star1.mt = mt[0];
#endif
        _star1.r  = r[0];
...
#ifdef BHMERGER_INTERFACE_LINKED
        // ===================== BHMERGER-ZJ: remnant kick and spin output =====================
        // Particle 1 is the retained remnant and particle 2 is removed by merge_().
        // Store spin as a change so a zero-initialized StarParameterOut remains a no-op.
        if (is_black_hole_merger && remnant_zj_success
            && _star1.mt > 0.0 && _star2.mt == 0.0) {
            double spin_direction_norm = 0.0;
            for (int k=0; k<3; k++) {
                spin_direction_norm += remnant_zj.spindirection_nor[k]
                                     * remnant_zj.spindirection_nor[k];
            }
            spin_direction_norm = std::sqrt(spin_direction_norm);
            const bool remnant_spin_valid =
                std::isfinite(remnant_zj.spin) && remnant_zj.spin >= 0.0
            ...
            for (int k=0; k<3; k++) {
                ...
                if (remnant_spin_valid) {
                    const double remnant_spin_component =
                        remnant_zj.spindirection_nor[k]
                        * remnant_zj.spin / spin_direction_norm;
                    _out1.spin[k] = remnant_spin_component - _spin1[k];
                }
            }
            vkick[3] = std::sqrt(vkick[0]*vkick[0]
                               + vkick[1]*vkick[1]
                               + vkick[2]*vkick[2]);
        }
        // =================== BHMERGER-ZJ: remnant kick and spin output end ===================
#endif
        for (int k=0; k<4; k++) _out1.vkick[k]=vkick[k];
        for (int k=0; k<4; k++) _out2.vkick[k]=vkick[k+4];
